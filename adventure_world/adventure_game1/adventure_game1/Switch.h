...
public:
	...
	void save(std::ostream& out) const {out << x << " " << y << " " << is_on << " " << is_active << " " << is_needed << " " << target_door_id << std::endl;}

	//Load the switch state
	void load(std::istream& in) {in >> x >> y >> is_on >> is_active >> is_needed >> target_door_id;}
 };
