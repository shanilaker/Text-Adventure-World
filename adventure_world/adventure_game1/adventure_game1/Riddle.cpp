...
void Riddle::checkRiddleAnswer(Screen& cur_screen, char key, vector<Player>& players, Game& the_game)
{
    for (auto& riddle : cur_screen.get_riddles())
    {
        if (riddle.getActivated())
        {
            if (key == riddle.getAnswer())
            {
                ...
                the_game.recordEvent(GameEvent::RIDDLE_ANSWER, (int)key);
                the_game.recordEvent(GameEvent::RIDDLE_RESULT, 1);
                cls();
                cur_screen.draw();
                for (auto& p : players) {
                    if (p.getIsActive()) {
            ...
            else
            {
                ...
                the_game.recordEvent(GameEvent::RIDDLE_ANSWER, (int)key);
                the_game.recordEvent(GameEvent::RIDDLE_RESULT, 0);
                cls();
                cur_screen.draw();
                for (auto& p : players) {
                    if (p.getIsActive())
