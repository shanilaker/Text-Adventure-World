...
public:
	...
	//Saves the bomb state
	void save(std::ostream& out) const {out << x << " " << y << " " << is_activated << " " << is_active << " " << time_to_explode << std::endl;}

	//Load the bomb state
	void load(std::istream& in) {in >> x >> y >> is_activated >> is_active >> time_to_explode;}
};
